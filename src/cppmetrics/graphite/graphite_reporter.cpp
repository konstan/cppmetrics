    ...
    const std::string &name, core::TimerPtr timer, uint64_t timestamp)
{
    ...
    sender_->send(prefix(name, "p75"),
        format(convertDurationUnit(snapshot->get75thPercentile())), timestamp);
    sender_->send(prefix(name, "p95"),
        format(convertDurationUnit(snapshot->get95thPercentile())), timestamp);
    sender_->send(prefix(name, "p98"),
        format(convertDurationUnit(snapshot->get98thPercentile())), timestamp);
    sender_->send(prefix(name, "p99"),
        format(convertDurationUnit(snapshot->get99thPercentile())), timestamp);
    ...
    const std::string &name, core::HistogramPtr histogram, uint64_t timestamp)
{
    ...
    sender_->send(
        prefix(name, "p75"), format(snapshot->get75thPercentile()), timestamp);
    sender_->send(
        prefix(name, "p95"), format(snapshot->get95thPercentile()), timestamp);
    sender_->send(
        prefix(name, "p98"), format(snapshot->get98thPercentile()), timestamp);
    sender_->send(
        prefix(name, "p99"), format(snapshot->get99thPercentile()), timestamp);
